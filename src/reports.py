        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            try:
                if isinstance(result, pd.DataFrame):
                    ...
                    # Преобразуем все datetime колонки в строковый формат
                    for col in result_copy.select_dtypes(include=["datetime64[ns]"]):
                        result_copy[col] = result_copy[col].dt.strftime("%d.%m.%Y %H:%M:%S")

                    json.dump(
                        result_copy.to_dict(orient="records"),
                        open(report_filename, "w", encoding="utf-8"),
                        ensure_ascii=False,
                        indent=2,
                    )
                else:
